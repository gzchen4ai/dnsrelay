...
void toDNSRecord(DNSRecord *retp, char *src, char *st, char **retPtr)
{
	...
	{
		char *str =
			(char *)malloc((retp->rdlength + 1) *
				       sizeof(char)); /* add 1 bytes for '\0' */
...
char *fromDNSRecord(DNSRecord *recordPtr, int *len)
{
	...
	*(sptr++) = htons(recordPtr->rdlength);
	...
	{
		cptr = (char *)sptr;
		memcpy(cptr, recordPtr->rdata,
		       recordPtr->rdlength * sizeof(char));
...
void unpackDNSRecords(DNSPacket *packetPtr, DNSRecords *recordsPtr)
{
	...
	recordsPtr->record = (DNSRecord *)malloc((packetPtr->header->ancount) *
						 sizeof(DNSRecord));
...
void debugDNSRecords(DNSRecords *recordsPtr)
{
	...
	for (int i = 0; i < recordsPtr->len; i++) {
		printf("[%d] %#x %d %#x\n", i, recordsPtr->record[i].qname[0],
		       recordsPtr->record[i].ttl,
		       recordsPtr->record[i].rdata[0]);
	}
