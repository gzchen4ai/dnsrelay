...
void cache_add(cache *a, char *address, DNSRecords *reports, int ttl)
{
	...
	a->address[a->head] =
		(char *)malloc(sizeof(char) * (strlen(address) + 1));
...
DNSRecords *get_records(char *address)
{
	...
	if (list_find_records(&List, address, records)) {
		...
		DNSRecord *record2 =
			(DNSRecord *)malloc(sizeof(DNSRecord) * (records->len));
		memcpy(record2, records->record,
		       sizeof(DNSRecord) * (records->len));
